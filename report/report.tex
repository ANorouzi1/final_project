...
\qquad {\small\href{https://github.com/ANorouzi1/final_project/tree/main}{Project code}}}
...
\begin{tikzpicture}[
  >=Latex,
  box/.style={draw, rounded corners=1pt, align=center, minimum height=5.2mm, inner sep=1.2pt, font=\scriptsize}
]
  \node[box, fill=gray!12, text width=23mm] (shared) at (-2.15,0)
    {Shared U-Net\\decoder features};
  \node[box, fill=cvprblue!13, text width=35mm] (mask) at (1.25,0.39)
    {Mask head $\rightarrow$ field probability\\\tiny used at inference};
  \node[box, fill=orange!14, text width=35mm] (auxiliary) at (1.25,-0.39)
    {Auxiliary head $\rightarrow$ SDF or border\\\tiny separate variants; training only};
  \draw[->, line width=.45pt] (shared.east) -- (mask.west);
  \draw[->, line width=.45pt] (shared.east) -- (auxiliary.west);
\end{tikzpicture}
\vspace{-5pt}
\end{center}
...
The second intervention instantiates the auxiliary branch as a distance head $z^D$. A $\tanh$ output matches the normalized SDF range, while Smooth L1 limits large-distance errors:
\begin{align}
...
The final intervention instantiates the same branch as a boundary head for $B_i=\mathbf{1}[|D_i|\leq0.12]$; mask training retains weighted BCE and Dice supervision.

Thin-band BCE uses the batch-wise negative-to-positive pixel ratio, clipped at 50. The joint objective is
\begin{equation}
  \mathcal{L}_{M+B}=\mathcal{L}_{\mathrm{WBCE}}+
  \mathcal{L}_{\mathrm{Dice}}+2\mathcal{L}^{\mathrm{bal}}_{\mathrm{BCE}}(z^B,B).
\end{equation}
The auxiliary head affects the mask only through shared features and the joint loss; mask inference is unchanged.
...
\textbf{Arian Norouzi:} pipeline, baseline, SDF objectives, evaluation/visuals. \textbf{Xiyang Lyu:} configurations, training/checkpoints, comparisons, and boundary-head experiments. \textbf{Yelyzaveta Tryfonova:} loader, weighted BCE, SDF caching/runtime, and debugging.
